...
pub fn cache_path(cache_root: &Path, item: &MediaItem, kind: &str) -> PathBuf {
    match (item.kind.as_str(), kind) {
        ...
        ("special"|"collection", "thumbnail") => cache_root.join("specials").join(&item.id).join("thumbnail.webp"),
...
fn local_image(item: &MediaItem, kind: &str) -> Option<PathBuf> {
    ...
    } else if kind == "thumbnail" && (item.kind == "episode" || item.kind == "special"||item.kind=="collection") {
...
pub fn ensure(cache_root: &Path, item: &MediaItem, kind: &str) -> Option<PathBuf> {
    ...
    if item.kind == "episode" || item.kind == "special"||item.kind=="collection" {
