use crate::{
    app_state::{persist_settings, Shared},
    artwork, database, ibroadcast, library, metadata, metadata_view,
    models::{
        EnrichedAnalyticsSummary, MediaItem, MetadataProviderStatus, MetadataSearchResult,
        Playlist, UserPreferences, UserProfile,
    },
    PORT,
};
...
pub struct SetupStatus {
    ...
    pub ibroadcast_client_id: Option<String>,
...
fn queue_auto_match(database_path: PathBuf, media: &[MediaItem]) {
    if !metadata::tmdb::configured() { return; }
    let mut ids = Vec::new();
    let mut shows = HashSet::new();
    for item in media {
        if item.kind == "movie" { ids.push(item.id.clone()); }
        else if shows.insert(item.show_title.clone().unwrap_or_default()) { ids.push(item.id.clone()); }
    }
    tauri::async_runtime::spawn(async move {
        for id in ids {
            if let Err(error) = metadata::tmdb::auto_match(&database_path, &id).await {
                eprintln!("TMDB auto-match skipped {id}: {error}");
            }
        }
    });
}
...
fn scan(state: &crate::app_state::AppState) -> Result<Vec<MediaItem>, String> {
    ...
    queue_auto_match(state.database_path.clone(), &media);
...
fn show_root(item: &MediaItem) -> PathBuf {
    ...
    let name = parent.file_name().and_then(|s| s.to_str()).unwrap_or("").to_lowercase();
    let looks_like_season = name.starts_with("season") || (name.chars().next() == Some('s') && name.get(1..).unwrap_or("").chars().all(|c| c.is_ascii_digit()));
    if looks_like_season { parent.parent().unwrap_or(parent).to_path_buf() } else { parent.to_path_buf() }
}

fn ensure_user(state: &crate::app_state::AppState, user_id: &str) -> Result<(), String> {
    if database::user_exists(&state.database_path, user_id) { Ok(()) } else { Err("User profile not found".into()) }
}
...
pub fn set_ibroadcast_client_id(client_id: String, state: TauriState<'_, Shared>) -> Result<(), String> {
    let clean = client_id.trim().to_string();
    state.settings.write().map_err(|_| "Settings lock poisoned")?.ibroadcast_client_id = if clean.is_empty() { None } else { Some(clean) };
    persist_settings(&state)
}
#[tauri::command]
pub fn set_library_path(path: String, state: TauriState<'_, Shared>) -> Result<(), String> {
    ...
    persist_settings(&state)?; scan(&state)?; Ok(())
}
#[tauri::command]
pub fn set_movie_path(path: String, state: TauriState<'_, Shared>) -> Result<(), String> {
    validate_folder(&path)?;
    let mut settings = state.settings.write().map_err(|_| "Settings lock poisoned")?; settings.movie_path = Some(path); drop(settings);
    persist_settings(&state)?; scan(&state)?; Ok(())
}
#[tauri::command]
pub fn set_tv_path(path: String, state: TauriState<'_, Shared>) -> Result<(), String> {
    validate_folder(&path)?;
    let mut settings = state.settings.write().map_err(|_| "Settings lock poisoned")?; settings.tv_path = Some(path); drop(settings);
    persist_settings(&state)?; scan(&state)?; Ok(())
}
#[tauri::command]
pub fn set_access_password(password: String, state: TauriState<'_, Shared>) -> Result<(), String> {
    ...
    state.settings.write().map_err(|_| "Settings lock poisoned")?.access_password_hash = Some(hash_password(&password)?);
...
#[tauri::command]
pub fn rename_user(user_id: String, name: String, state: TauriState<'_, Shared>) -> Result<Vec<UserProfile>, String> {
    ensure_user(&state, &user_id)?;
    let clean = name.trim(); if clean.is_empty() { return Err("User name cannot be empty".into()); }
    Connection::open(&state.database_path).map_err(|e| e.to_string())?.execute("UPDATE users SET name=?2 WHERE id=?1", params![user_id, clean]).map_err(|e| e.to_string())?;
    database::list_users(&state.database_path)
}
...
pub fn save_progress(user_id: String, id: String, seconds: u64, watched_seconds: Option<u64>, state: TauriState<'_, Shared>) -> Result<(), String> {
    ensure_user(&state, &user_id)?; database::save_progress(&state.database_path, &user_id, &id, seconds, watched_seconds.unwrap_or(0))
}
#[tauri::command]
pub fn reset_watch_status(user_id: String, ids: Vec<String>, state: TauriState<'_, Shared>) -> Result<Vec<MediaItem>, String> {
    ensure_user(&state, &user_id)?; database::reset_progress(&state.database_path, &user_id, &ids)?;
...
pub fn set_hidden(user_id: String, target_type: String, target_key: String, hidden: bool, state: TauriState<'_, Shared>) -> Result<Vec<MediaItem>, String> {
    ensure_user(&state, &user_id)?; database::set_hidden(&state.database_path, &user_id, &target_type, &target_key, hidden)?;
    enrich(&state, database::load_library_for_user(&state.database_path, &user_id, false)?)
}
#[tauri::command]
pub fn identify_item(id: String, identity: IdentityInput, state: TauriState<'_, Shared>) -> Result<Vec<MediaItem>, String> {
    let value = database::IdentityOverride {
        title: identity.title.filter(|v| !v.trim().is_empty()), year: identity.year,
        kind: identity.kind.filter(|v| v == "movie" || v == "episode"),
        show_title: identity.show_title.filter(|v| !v.trim().is_empty()), season: identity.season, episode: identity.episode,
    };
    database::save_identity_override(&state.database_path, &id, &value)?; scan(&state)
}
#[tauri::command]
pub fn reset_identification(id: String, state: TauriState<'_, Shared>) -> Result<Vec<MediaItem>, String> { database::clear_identity_override(&state.database_path, &id)?; scan(&state) }
#[tauri::command]
pub fn identify_show(id: String, show_title: String, state: TauriState<'_, Shared>) -> Result<Vec<MediaItem>, String> {
    let title = show_title.trim(); if title.is_empty() { return Err("Show title cannot be empty".into()); }
    let item = state.media.read().map_err(|_| "Media lock poisoned")?.iter().find(|i| i.id == id).cloned().ok_or("Media item not found")?;
    if item.kind != "episode" { return Err("Selected item is not a TV episode".into()); }
    database::save_show_override(&state.database_path, &show_root(&item).to_string_lossy(), title)?; scan(&state)
}
...
#[tauri::command]
pub fn create_playlist(user_id: String, name: String, state: TauriState<'_, Shared>) -> Result<Vec<Playlist>, String> { ensure_user(&state, &user_id)?; database::create_playlist(&state.database_path, &user_id, &name)?; database::list_playlists(&state.database_path, &user_id) }
#[tauri::command]
pub fn add_to_playlist(user_id: String, playlist_id: String, media_id: String, state: TauriState<'_, Shared>) -> Result<Vec<Playlist>, String> { ensure_user(&state, &user_id)?; database::add_playlist_item(&state.database_path, &user_id, &playlist_id, &media_id)?; database::list_playlists(&state.database_path, &user_id) }
#[tauri::command]
pub fn remove_from_playlist(user_id: String, playlist_id: String, media_id: String, state: TauriState<'_, Shared>) -> Result<Vec<Playlist>, String> { ensure_user(&state, &user_id)?; database::remove_playlist_item(&state.database_path, &user_id, &playlist_id, &media_id)?; database::list_playlists(&state.database_path, &user_id) }
#[tauri::command]
pub fn delete_playlist(user_id: String, playlist_id: String, state: TauriState<'_, Shared>) -> Result<Vec<Playlist>, String> { ensure_user(&state, &user_id)?; database::delete_playlist(&state.database_path, &user_id, &playlist_id)?; database::list_playlists(&state.database_path, &user_id) }
...
#[tauri::command]
pub async fn ibroadcast_device_poll(user_id: String, device_code: String, state: TauriState<'_, Shared>) -> Result<ibroadcast::DevicePollResponse, String> { ensure_user(&state, &user_id)?; ibroadcast::device_poll(&state, &user_id, &device_code).await }
#[tauri::command]
pub async fn ibroadcast_sync(user_id: String, state: TauriState<'_, Shared>) -> Result<ibroadcast::IbLibrary, String> { ensure_user(&state, &user_id)?; ibroadcast::sync_library(&state, &user_id).await }
#[tauri::command]
pub fn ibroadcast_library(user_id: String, state: TauriState<'_, Shared>) -> Result<ibroadcast::IbLibrary, String> { ensure_user(&state, &user_id)?; ibroadcast::load_library(&state.provider_path, &user_id) }
#[tauri::command]
pub fn ibroadcast_disconnect(user_id: String, state: TauriState<'_, Shared>) -> Result<(), String> { ensure_user(&state, &user_id)?; ibroadcast::disconnect(&state, &user_id) }

#[tauri::command]
pub fn metadata_provider_status() -> Vec<MetadataProviderStatus> {
    vec![
        metadata::tmdb::status(),
        MetadataProviderStatus { provider: "tvdb".into(), configured: false, enabled: false, primary: false, attribution: "TheTVDB integration is optional and not configured in this build.".into() },
    ]
}
...
pub async fn metadata_search(id: String, query: Option<String>, state: TauriState<'_, Shared>) -> Result<Vec<MetadataSearchResult>, String> {
    let item = state.media.read().map_err(|_| "Media lock poisoned")?.iter().find(|m| m.id == id).cloned().ok_or("Media item not found")?;
    let q = query.filter(|q| !q.trim().is_empty()).unwrap_or_else(|| if item.kind == "episode" { item.show_title.clone().unwrap_or(item.title.clone()) } else { item.title.clone() });
    metadata::tmdb::search(if item.kind == "episode" { "series" } else { "movie" }, &q, item.year).await
}
#[tauri::command]
...
pub async fn metadata_auto_match_all(state: TauriState<'_, Shared>) -> Result<u32, String> {
    ...
    let mut ids = Vec::new(); let mut shows = HashSet::new();
    for item in media { if item.kind == "movie" { ids.push(item.id); } else if shows.insert(item.show_title.unwrap_or_default()) { ids.push(item.id); } }
    let mut matched = 0;
    for id in ids { if metadata::tmdb::auto_match(&state.database_path, &id).await.unwrap_or(false) { matched += 1; } }
    Ok(matched)
}
...
pub fn server_status(state: TauriState<'_, Shared>) -> Result<ServerStatus, String> {
    ...
    Ok(ServerStatus {
        running: true, local_url: lan_url(), library_path: settings.library_path.clone(), movie_path: settings.movie_path.clone(), tv_path: settings.tv_path.clone(), item_count,
        ffprobe_available: command_available("ffprobe"), ffmpeg_available: command_available("ffmpeg"), access_password_set: settings.access_password_hash.is_some(),
        artwork_cache_bytes: artwork::cache_size(&state.artwork_path), setup_complete: settings.setup_complete, ibroadcast_client_id: settings.ibroadcast_client_id.clone(),
    })
}
