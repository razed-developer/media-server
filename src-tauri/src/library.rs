...
pub fn find_poster(path: &Path) -> Option<PathBuf> {
    ...
    for extension in IMAGE_EXTENSIONS { candidates.push(parent.join(format!("{stem}.{extension}"))); }
    for name in ["poster", "folder", "cover"] { for extension in IMAGE_EXTENSIONS { candidates.push(parent.join(format!("{name}.{extension}"))); } }
    if let Some(show_root) = parent.parent() { for name in ["poster", "folder", "cover"] { for extension in IMAGE_EXTENSIONS { candidates.push(show_root.join(format!("{name}.{extension}"))); } } }
    ...
    if let Ok(entries) = fs::read_dir(parent) {
        for entry in entries.flatten() {
            ...
            tracks.push(SubtitleTrack {
                ...
                stream_index: None, embedded: false, format: Some(extension), forced: false, default: false,
            });
        }
    }
    tracks
...
pub fn scan(root: &Path, database_path: &Path, kind_hint: Option<&str>) -> Result<Vec<MediaItem>, String> {
    ...
    for entry in WalkDir::new(root).follow_links(false).into_iter().flatten().filter(|e| e.file_type().is_file()) {
        ...
        let id = make_id(path);
        let parsed = match kind_hint { Some("movie") => naming::parse_movie(path), Some("episode") => naming::parse_tv(path), _ => naming::parse(path) };
        ...
        subtitles.extend(info.subtitles.iter().map(|track| SubtitleTrack {
            label: track.title.clone().unwrap_or_else(|| track.language.to_uppercase()), language: track.language.clone(),
            url: Some(format!("/subtitle/{id}/embedded/{}", track.index)), stream_index: Some(track.index), embedded: true,
            format: track.codec.clone(), forced: track.forced, default: track.default,
        }));
        let is_episode = parsed.kind == "episode";
        media.push(MediaItem {
            id: id.clone(), title: parsed.title, year: parsed.year, kind: parsed.kind, show_title: parsed.show_title,
            season: parsed.season, episode: parsed.episode, episode_end: parsed.episode_end, path: path.to_string_lossy().to_string(),
            stream_url: format!("/play/{id}"), poster_url: Some(format!("/art/{id}/poster")),
            backdrop_url: Some(format!("/art/{id}/backdrop")),
            thumbnail_url: is_episode.then(|| format!("/art/{id}/thumbnail")),
            subtitles, progress_seconds: *progress.get(&id).unwrap_or(&0), duration_seconds: info.duration_seconds,
            container: info.container.clone(), video_codec: info.video_codec.clone(), audio_codec: info.audio_codec.clone(),
            width: info.width, height: info.height, playback_mode,
        });
    }
    media.sort_by(|a, b| {
