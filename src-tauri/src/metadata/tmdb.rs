...
async fn get_json(url: &str, params: &[(&str, String)]) -> Result<Value, String> {
    ...
    if !response.status().is_success() { return Err(format!("TMDB request failed ({})", response.status())); }
...
pub async fn search(kind: &str, query: &str, year: Option<u16>) -> Result<Vec<MetadataSearchResult>, String> {
    ...
    let json = get_json(&format!("{API}/search/{endpoint}"), &params).await?;
    let mut out = vec![];
    for item in json.get("results").and_then(Value::as_array).into_iter().flatten().take(20) {
        let title = item.get(if endpoint == "movie" { "title" } else { "name" }).and_then(Value::as_str).unwrap_or("Untitled").to_string();
        let date = item.get(if endpoint == "movie" { "release_date" } else { "first_air_date" }).and_then(Value::as_str);
        out.push(MetadataSearchResult {
            provider: "tmdb".into(), provider_id: item.get("id").and_then(Value::as_i64).unwrap_or_default().to_string(),
            entity_type: if endpoint == "movie" { "movie".into() } else { "series".into() }, title, year: year_from(date),
            overview: item.get("overview").and_then(Value::as_str).map(str::to_string),
            poster_url: image_url(item.get("poster_path").and_then(Value::as_str), "w342"),
            backdrop_url: image_url(item.get("backdrop_path").and_then(Value::as_str), "w780"),
            rating: item.get("vote_average").and_then(Value::as_f64),
        });
    }
    Ok(out)
}
...
fn detail_entity(base: &MetadataEntity, value: &Value, kind: &str) -> MetadataEntity {
    let title = value.get(if kind == "movie" { "title" } else { "name" }).and_then(Value::as_str).unwrap_or(&base.title).to_string();
    let date = value.get(if kind == "movie" { "release_date" } else { "first_air_date" }).and_then(Value::as_str).map(str::to_string);
    MetadataEntity {
        id: base.id.clone(), entity_type: base.entity_type.clone(), parent_id: base.parent_id.clone(), title,
        original_title: value.get(if kind == "movie" { "original_title" } else { "original_name" }).and_then(Value::as_str).map(str::to_string),
        year: year_from(date.as_deref()).or(base.year), overview: value.get("overview").and_then(Value::as_str).filter(|s| !s.is_empty()).map(str::to_string), release_date: date,
        runtime_minutes: if kind == "movie" { value.get("runtime").and_then(Value::as_u64).map(|n| n as u32) } else { value.get("episode_run_time").and_then(Value::as_array).and_then(|a| a.first()).and_then(Value::as_u64).map(|n| n as u32) },
        season_number: base.season_number, episode_number: base.episode_number, genres: genres(value), rating: value.get("vote_average").and_then(Value::as_f64),
        poster_path: value.get("poster_path").and_then(Value::as_str).map(str::to_string), backdrop_path: value.get("backdrop_path").and_then(Value::as_str).map(str::to_string), still_path: base.still_path.clone(), metadata_json: value.clone(),
    }
}
...
pub async fn apply_match(db: &Path, media_id: &str, provider_id: &str, matched_by: &str, locked: bool) -> Result<(), String> {
    ...
    if entity.entity_type == "movie" {
        ...
        replace_entity_metadata(db, &detail_entity(&entity, &detail, "movie"))?;
        set_provider_match(db, &ProviderMatch { entity_id: entity.id, provider: "tmdb".into(), provider_id: provider_id.into(), matched_by: matched_by.into(), confidence: None, locked })?;
        return Ok(());
    }
    ...
    replace_entity_metadata(db, &detail_entity(&series, &detail, "series"))?;
    set_provider_match(db, &ProviderMatch { entity_id: series.id.clone(), provider: "tmdb".into(), provider_id: provider_id.into(), matched_by: matched_by.into(), confidence: None, locked })?;
...
pub async fn hydrate_series_children(db: &Path, series_id: &str, provider_id: &str) -> Result<(), String> {
    ...
    for (season_no, episodes) in local {
        let season_json = get_json(&format!("{API}/tv/{provider_id}/season/{season_no}"), &[("language", "en-US".into())]).await?;
        if let Some(season) = children(db, series_id, "season")?.into_iter().find(|s| s.season_number == Some(season_no)) {
            let mut updated = season.clone(); updated.title = season_json.get("name").and_then(Value::as_str).unwrap_or(&season.title).to_string();
            updated.overview = season_json.get("overview").and_then(Value::as_str).filter(|s| !s.is_empty()).map(str::to_string); updated.release_date = season_json.get("air_date").and_then(Value::as_str).map(str::to_string); updated.poster_path = season_json.get("poster_path").and_then(Value::as_str).map(str::to_string); updated.metadata_json = season_json.clone(); replace_entity_metadata(db, &updated)?;
            let provider_eps: HashMap<u16, &Value> = season_json.get("episodes").and_then(Value::as_array).into_iter().flatten().filter_map(|e| e.get("episode_number").and_then(Value::as_u64).map(|n| (n as u16, e))).collect();
            for episode in episodes {
                if let Some(source) = provider_eps.get(&episode.episode_number.unwrap_or(0)) {
                    let mut updated = episode.clone(); updated.title = source.get("name").and_then(Value::as_str).unwrap_or(&episode.title).to_string();
                    updated.overview = source.get("overview").and_then(Value::as_str).filter(|s| !s.is_empty()).map(str::to_string); updated.release_date = source.get("air_date").and_then(Value::as_str).map(str::to_string); updated.runtime_minutes = source.get("runtime").and_then(Value::as_u64).map(|v| v as u32); updated.rating = source.get("vote_average").and_then(Value::as_f64); updated.still_path = source.get("still_path").and_then(Value::as_str).map(str::to_string); updated.metadata_json = (*source).clone(); replace_entity_metadata(db, &updated)?;
                }
            }
        }
    }
...
fn confidence(query: &str, year: Option<u16>, result: &MetadataSearchResult) -> f64 {
    let a = normalize(query); let b = normalize(&result.title);
    let title: f64 = if a == b { 0.88 } else if a.contains(&b) || b.contains(&a) { 0.72 } else { 0.45 };
    let year_score: f64 = match (year, result.year) { (Some(a), Some(b)) if a == b => 0.10, (Some(a), Some(b)) if a.abs_diff(b) <= 1 => 0.04, (None, _) => 0.02, _ => 0.0 };
    (title + year_score).min(0.98_f64)
}
