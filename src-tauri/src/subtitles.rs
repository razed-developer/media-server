...
fn parse_results(value:&Value,language:&str)->Vec<SubtitleSearchResult>{
 let mut out=vec![];
...
async fn search_request(c:&Credentials,token:&str,base:&str,params:&[(&str,String)],language:&str)->Result<Vec<SubtitleSearchResult>,String>{
 ...
 Ok(parse_results(&value,language))
}
...
pub async fn search(state:&crate::app_state::AppState,media_id:&str,language:&str)->Result<Vec<SubtitleSearchResult>,String>{
 ...
 let mut exact_name=common();exact_name.push(("query",filename_hint(&item)));searches.push(exact_name);
 ...
 for params in searches{
  match search_request(&c,&token,&base,&params,language).await{
 ...
 out.truncate(30);activity::info("Subtitles",format!("OpenSubtitles returned {} unique candidates, with exact metadata matches first",out.len()));Ok(out)
}
...
pub async fn download(state:&crate::app_state::AppState,media_id:&str,file_id:i64,language:&str)->Result<DownloadedSubtitle,String>{
 ...
 let response=client().post(format!("{base}/download")).header("Api-Key",&c.api_key).header(USER_AGENT,APP_USER_AGENT).header(AUTHORIZATION,format!("Bearer {token}")).json(&json!({"file_id":file_id,"sub_format":"srt"})).send().await.map_err(|e|e.to_string())?;let code=response.status();let value:Value=response.json().await.map_err(|e|e.to_string())?;if !code.is_success(){let message=value.get("message").and_then(Value::as_str).unwrap_or("download unavailable");activity::error("Subtitles",format!("OpenSubtitles download request failed ({code}): {message}"));return Err(format!("OpenSubtitles download request failed ({code}): {message}"))}let link=value.get("link").and_then(Value::as_str).ok_or("OpenSubtitles did not return a download link")?;
 ...
 if fs::write(&destination,&bytes).is_ok(){let file_name=destination.file_name().and_then(|v|v.to_str()).unwrap_or("subtitle.srt").to_string();let url=format!("/subtitle/{}/{}",urlencoding::encode(media_id),urlencoding::encode(&file_name));activity::info("Subtitles",format!("Saved {language} subtitle next to “{}” as {file_name}",item.title));return Ok(DownloadedSubtitle{file_name,language:language.to_string(),label:format!("{} · OpenSubtitles",language.to_uppercase()),url:Some(url),local_path:None})}
