...
function App() {
  ...
  const pauseForNavigation = () => { if (!selected) return; const current = Math.floor(videoRef.current?.currentTime ?? selected.progressSeconds); void saveProgress(true); videoRef.current?.pause(); const paused = { ...selected, progressSeconds: current }; setItems(existing => existing.map(item => item.id === paused.id ? { ...item, progressSeconds: current } : item)); setPausedMedia(paused); setSelected(null); setSubtitleChoice('off'); };
