...
function App() {
  ...
  const refresh = async () => {
    try {
      ...
      setItems(library); setStatus(serverStatus); setPlaylists(playlistData); setAnalytics(stats); setUsers(userData); setAvatars(Object.fromEntries(avatarData.map(avatar => [avatar.userId, avatar]))); applyTheme(prefs.theme); setError(null);
