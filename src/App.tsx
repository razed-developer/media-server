...
function App() {
  ...
  const playback = usePlaybackController({
    items: data.items, setItems: data.setItems, selected, setSelected, sessions: collections.sessions,
    markPlaying: collections.markPlaying, markIdle: collections.markIdle,
    requestCollectionUnlock: sourceId => { collections.setSelectedCollectionId(sourceId); setSection("collection"); data.setError("Unlock this collection before playing it."); },
  });
  const catalog = useLibraryCatalog({ items: data.items, hiddenItems: data.hiddenItems, sessions: collections.sessions, query, selectedShowTitle, playlists: data.playlists, selectedPlaylistId });
  const mediaActions = useMediaActions({ isDesktop, section, selected, setSelected, selectedShowTitle, setSelectedShowTitle, setItems: data.setItems, setHiddenItems: data.setHiddenItems, setError: data.setError });
  const playlistActions = usePlaylistsController({ playlists: data.playlists, setPlaylists: data.setPlaylists, selectedPlaylistId, setSelectedPlaylistId, dialog, showPlaylists: () => setSection("playlists"), setError: data.setError });
  ...
  useEffect(() => {
    ...
    const showLive = () => { if (selected) playback.closePlayer(); else setSelected(null); data.setError(null); };
    const back = () => { if (selected) playback.closePlayer(); };
    window.addEventListener("onyx-remote-live", showLive);
    window.addEventListener("onyx-remote-back", back);
    return () => { window.removeEventListener("onyx-remote-live", showLive); window.removeEventListener("onyx-remote-back", back); };
  }, [projectorMode, selected]);

  const navigate = (next: Section) => { if (selected) playback.pauseForNavigation(); setSection(next); setSelectedShowTitle(null); setSelectedPlaylistId(null); if (next !== "collection") collections.setSelectedCollectionId(null); setQuery(""); };
  const openCollection = (id: string) => { if (selected) playback.pauseForNavigation(); collections.setSelectedCollectionId(id); setSection("collection"); setQuery(""); data.setError(null); };
  ...
  const clearHistory = async () => { const ids = data.items.filter(item => item.lastWatchedAt || item.progressSeconds > 0).map(item => item.id); if (ids.length && window.confirm("Clear all watch history for this profile?")) await mediaActions.resetWatched(ids); };
  const scanOneLibrary = async (kind: "movie" | "tv" | "special" | `collection:${string}`, label: string) => { try { data.setError(`Scanning ${label}…`); await rescanLibraryKind(kind); await data.refresh(); data.setError(null); } catch (cause) { data.setError(String(cause)); } };
  const openRecommendation = (entry: RecommendationEntry) => { if (entry.targetType === "movie") { const movie = catalog.movies.find(item => socialKey(item) === entry.targetKey || item.title === entry.title); if (movie) playback.startPlayback(movie); } else { const show = catalog.shows.find(value => socialKey(value.representative) === entry.targetKey || value.title === entry.title); if (show) { setSection("tv"); setSelectedShowTitle(show.title); setQuery(""); } } };
  ...
  const player = selected ? <PlayerPage item={selected} videoRef={playback.videoRef} sourceUrl={playback.sourceUrl} subtitleChoice={playback.subtitleChoice} playableSubtitles={playback.playableSubtitles} episodeLabel={episodeLabel} onBack={playback.closePlayer} onPlay={() => collections.markPlaying(selected)} onPause={() => { void playback.saveProgress(true); collections.markIdle(selected); }} onEnded={() => { collections.markIdle(selected); if (projectorMode) playback.closePlayer(); }} onTimeUpdate={() => void playback.saveProgress()} onSubtitleChange={playback.changeSubtitle} social={!projectorMode && isDesktop && selected.kind === "movie" ? <SocialBar targetType="movie" targetKey={socialKey(selected)} title={selected.title} posterUrl={selected.posterUrl} users={data.users}/> : undefined}/> : null;
  ...
  const shell = projectorMode ? <div className="projector-shell">{data.error && <div className="error-banner">{data.error}</div>}<SleepTimer projector/>{player ?? (activeUser ? <LiveChannelsView media={data.items} onOpenSettings={() => undefined} projector userName={activeUser.name}/> : <div className="live-empty">Loading profile…</div>)}</div> : <div className={`app-shell ${isDesktop ? "desktop-shell" : ""}`}>
    ...
    {player ?? <main className="content">
      ...
      {section === "live" && <LiveChannelsView media={data.items} onOpenSettings={() => navigate("settings")}/>} {section === "music" && <MusicView/>}
      ...
      {section === "analytics" && <AnalyticsPage analytics={data.analytics}/>} {section === "settings" && <SettingsPage onChanged={data.refresh}/>} {section === "hidden" && <HiddenMediaPage movies={catalog.hiddenMovies} shows={catalog.hiddenShows} onPlay={playback.startPlayback} onMovieMenu={(event, item) => openMenu(event, { type: "item", item }, true)} onShowMenu={(event, show) => openMenu(event, { type: "show", show }, true)}/>}
    </main>}
