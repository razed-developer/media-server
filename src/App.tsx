...
function App() {
  ...
  const sidebar = <aside className="sidebar">
    ...
    {collections.map(source => <button key={source.id} className={section === 'collection' && selectedCollectionId === source.id ? 'active' : ''} onClick={() => openCollection(source.id)}>{source.protected ? <Lock size={19} /> : <FolderOpen size={19} />}{source.name}</button>)}
