...
function App() {
  ...
  const startPlayback = (item: MediaItem) => { let next = item; if (item.durationSeconds && item.progressSeconds > 0 && 1 - item.progressSeconds / item.durationSeconds <= .1) { const resume = window.confirm('Less than 10% remains.\n\nOK: continue where you left off\nCancel: restart from the beginning'); if (!resume) next = { ...item, progressSeconds: 0 }; } try{sessionStorage.setItem('onyx-current-media',JSON.stringify(next))}catch{/* best effort */} lastWatchTickRef.current = Date.now(); setPausedMedia(null); startTransition(() => setSelected(next)); setSubtitleChoice('off'); };
