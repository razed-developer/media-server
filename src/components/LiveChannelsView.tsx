...
export function LiveChannelsView({media,onOpenSettings,projector=false,userName}:{media:MediaItem[];onOpenSettings:()=>void;projector?:boolean;userName?:string}){
  ...
  {projector&&guide.length>0&&<section className="projector-channel-grid" aria-label="Live channels">{guide.map(row=>{const currentMedia=row.current?mediaById.get(row.current.mediaId):undefined;const visual=row.channel.artUrl??(!row.channel.artIcon?(currentMedia?.backdropUrl??currentMedia?.thumbnailUrl??currentMedia?.posterUrl):undefined);return <button autoFocus={row.channel.id===(tuned?.channel.id??guide[0]?.channel.id)} className={`projector-channel ${tuned?.channel.id===row.channel.id?'tuned':''}`} key={row.channel.id} onClick={()=>tune(row)}>{visual?<img src={resolveMediaUrl(visual)} alt=""/>:<ChannelArtwork channel={row.channel} className="projector-channel-placeholder"/>}<span><strong>{row.channel.name}</strong><b>{row.current?.title??'Guide loading…'}</b>{row.current?.subtitle&&<small>{row.current.subtitle}</small>}</span></button>})}</section>}
