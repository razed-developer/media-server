...
import { Captions, CaptionsOff, Expand, LoaderCircle, Radio, RefreshCw, Settings2 } from 'lucide-react';
...
export function LiveChannelsView({media,onOpenSettings,projector=false,userName}:{media:MediaItem[];onOpenSettings:()=>void;projector?:boolean;userName?:string}){
  ...
  {projector&&tuned?.current&&<button className="projector-fullscreen" onClick={()=>void toggleFullscreen()}><Expand size={22}/>Watch fullscreen</button>}
  {projector&&guide.length>0&&<section className="projector-channel-grid" aria-label="Live channels">{guide.map(row=>{const currentMedia=row.current?mediaById.get(row.current.mediaId):undefined;const visual=row.channel.artUrl??currentMedia?.backdropUrl??currentMedia?.thumbnailUrl??currentMedia?.posterUrl;return <button autoFocus={row.channel.id===(tuned?.channel.id??guide[0]?.channel.id)} className={`projector-channel ${tuned?.channel.id===row.channel.id?'tuned':''}`} key={row.channel.id} onClick={()=>tune(row)}>{visual?<img src={resolveMediaUrl(visual)} alt=""/>:<span className="projector-channel-placeholder"><Radio size={34}/></span>}<span><strong>{row.channel.name}</strong><b>{row.current?.title??'Guide loading…'}</b>{row.current?.subtitle&&<small>{row.current.subtitle}</small>}</span></button>})}</section>}
