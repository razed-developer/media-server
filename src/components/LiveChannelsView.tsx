...
export function LiveChannelsView({media,onOpenSettings}:{media:MediaItem[];onOpenSettings:()=>void}){
  ...
  const refresh=async(autoTune=false)=>{setLoadingGuide(true);try{const next=await getLiveChannelGuide();localStorage.setItem(cacheKey(),JSON.stringify(next));setGuide(next);setError(null);setTuned(current=>{if(!current&&autoTune)return next.find(row=>Boolean(row.current))??null;if(!current)return current;return next.find(row=>row.channel.id===current.channel.id)??null})}catch(c){setError(String(c))}finally{setLoadingGuide(false);setInitializing(false)}};

  useEffect(()=>{let disposed=false;let timer:number|undefined;const start=async()=>{try{const channels=await listLiveChannels();if(disposed)return;const known=new Set(channels.map(channel=>channel.id));const useful=reviveRows(cached.filter(row=>known.has(row.channel.id)));const cacheMatches=useful.length===channels.length&&channels.length>0;setGuide(cacheMatches?useful:placeholders(channels));if(cacheMatches&&!tuned)setTuned(useful.find(row=>Boolean(row.current))??null);setInitializing(false);if(channels.length&&(!cacheMatches||!guideHasRoom(useful)))await refresh(true);timer=window.setInterval(()=>{setGuide(current=>{const revived=reviveRows(current);if(!guideHasRoom(revived))void refresh(false);return revived});setTuned(current=>current?reviveRows([current])[0]:current)},60_000)}catch(c){if(!disposed){setError(String(c));setInitializing(false)}}};void start();return()=>{disposed=true;if(timer)window.clearInterval(timer)}},[]);
  ...
  return <div className="live-page"><section className="live-header"><div><p className="eyebrow">LIVE CHANNELS</p><h1>What’s on now</h1></div><div className="live-header-actions">{loadingGuide&&guide.length>0&&<span className="live-guide-status"><LoaderCircle className="spin" size={14}/>Extending guide…</span>}<button className="live-icon-button" title="Refresh guide" aria-label="Refresh guide" onClick={()=>void refresh(false)}><RefreshCw size={17}/></button><button className="live-icon-button" title="Manage channels" aria-label="Manage channels" onClick={onOpenSettings}><Settings2 size={17}/></button></div></section>
  {error&&<div className="error-banner">{error}</div>}{tuned?.current&&<section className="live-player-card"><div className="live-player-info" style={(tuned.channel.artUrl||fallbackArt)?{backgroundImage:`linear-gradient(90deg,rgba(5,7,10,.22),rgba(5,7,10,.92)),url(${resolveMediaUrl(tuned.channel.artUrl||fallbackArt)})`}:undefined}><div><span className="live-badge"><i/>LIVE</span><p>{tuned.channel.name}</p><button className="live-library-link" title="Open this title in your library" onClick={()=>openLibraryItem(tuned.current!.mediaId)}><h2>{tuned.current.title}</h2>{tuned.current.subtitle&&<span>{tuned.current.subtitle}</span>}</button></div></div><div ref={videoShellRef} className={`live-video ${fullscreen?'is-fullscreen':''}`} onClick={()=>void toggleFullscreen()} title={fullscreen?'Click to return to guide':'Click to watch fullscreen'}><video ref={videoRef} key={`${tuned.channel.id}:${tuned.current.mediaId}:${tuned.current.startsAt}`} autoPlay playsInline src={stream} onPause={keepLive} onLoadedMetadata={keepLive} onEnded={()=>{setGuide(current=>reviveRows(current));const row=guide.find(item=>item.channel.id===tuned.channel.id);if(row)setTuned(reviveRows([row])[0])}}>{subtitleTracks.map((track,index)=><track key={`${track.label}:${index}`} kind="subtitles" src={resolveMediaUrl(track.url)} srcLang={track.language||'und'} label={track.label||track.language||`Subtitle ${index+1}`}/>)}</video>{subtitleTracks.length>0&&<button className={`live-caption-toggle ${subtitlesOn?'active':''}`} title={subtitlesOn?'Turn subtitles off':'Turn subtitles on'} aria-label={subtitlesOn?'Turn subtitles off':'Turn subtitles on'} onClick={event=>{event.stopPropagation();setSubtitlesOn(value=>!value)}}>{subtitlesOn?<Captions size={20}/>:<CaptionsOff size={20}/>}</button>}<span className="live-fullscreen-hint">{fullscreen?'Click video to return to guide':'Click video for fullscreen'}</span></div></section>}
  {initializing&&guide.length===0?<div className="live-empty"><LoaderCircle className="spin"/><span>Loading channels…</span></div>:guide.length===0?<div className="live-empty"><Radio size={34}/><h2>No channels yet</h2><button className="primary" onClick={onOpenSettings}>Create a channel</button></div>:<section className="guide-shell"><div className="guide-header-row"><div className="guide-channel-heading">Channels</div><div className="guide-time-head">{ticks.map(tick=><span key={tick} style={{left:`${(tick-windowStart)/WINDOW_SECONDS*100}%`}}>{clock(tick)}</span>)}<i className="guide-now-line" style={{left:`${(now-windowStart)/WINDOW_SECONDS*100}%`}}/></div></div>{guide.map(row=>{const currentMedia=row.current?mediaById.get(row.current.mediaId):undefined;const showFallback=row.channel.criteriaType==='show'?(currentMedia?.backdropUrl??currentMedia?.thumbnailUrl??currentMedia?.posterUrl):undefined;const visual=row.channel.artUrl||showFallback;return <div className={`guide-row ${tuned?.channel.id===row.channel.id?'tuned':''}`} key={row.channel.id}><button className="guide-channel" onClick={()=>tune(row)}>{visual?<img src={resolveMediaUrl(visual)} alt=""/>:<span className="guide-channel-icon"><Radio size={20}/><b>{row.channel.name.slice(0,2).toUpperCase()}</b></span>}<div><strong>{row.channel.name}</strong></div></button><div className="guide-timeline" onClick={()=>tune(row)}>{row.programs.map(program=>{const start=Math.max(windowStart,program.startsAt),end=Math.min(windowEnd,program.endsAt);if(end<=windowStart||start>=windowEnd)return null;const left=(start-windowStart)/WINDOW_SECONDS*100,width=(end-start)/WINDOW_SECONDS*100;return <button key={`${program.mediaId}:${program.startsAt}`} className={`guide-program ${row.current?.startsAt===program.startsAt?'current':''}`} style={{left:`${left}%`,width:`${Math.max(width,.75)}%`}} onClick={event=>{event.stopPropagation();tune(row)}}><strong>{program.title}</strong><small>{program.subtitle??`${clock(program.startsAt)}–${clock(program.endsAt)}`}</small></button>})}<i className="guide-now-line" style={{left:`${(now-windowStart)/WINDOW_SECONDS*100}%`}}/></div></div>})}</section>}</div>;
}
