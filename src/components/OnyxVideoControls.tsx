...
export function OnyxVideoControls(){
 const[video,setVideo]=useState<HTMLVideoElement|null>(null);const[item,setItem]=useState<MediaItem>();const[current,setCurrent]=useState(0);const[playing,setPlaying]=useState(false);const[volume,setVolume]=useState(1);const[visible,setVisible]=useState(true);const hideTimer=useRef<number|undefined>(undefined);
 ...
 if(!video||!controls)return null;const stage=video.closest<HTMLElement>('.video-stage');return stage?createPortal(controls,stage):null;
}
