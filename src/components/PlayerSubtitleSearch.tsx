...
export function PlayerSubtitleSearch(){
 ...
 const generate=async()=>{if(!item)return;setCaptionBusy(true);try{const queued=await generateCaptions(item.id,item.subtitles.length>0);if(queued)window.dispatchEvent(new Event('onyx-caption-refresh'));setCaptionMessage(queued?'AI subtitles queued':'Already queued')}catch(error){setCaptionMessage(String(error).replace(/^Error:\s*/,''))}finally{setCaptionBusy(false)}};
