...
export function RecommendationsRail({userId,onOpen}:{userId:string;onOpen:(entry:RecommendationEntry)=>void}){
 ...
 const open=async(entry:RecommendationEntry)=>{if(!entry.read){await markRecommendationRead(userId,entry.id);setEntries(current=>current.map(value=>value.id===entry.id?{...value,read:true}:value))}onOpen(entry)};
