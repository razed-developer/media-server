...
export interface DownloadedSubtitle{fileName:string;language:string;label:string;url:string;}
...
export async function downloadOnlineSubtitle(mediaId:string,fileId:number,language:string):Promise<DownloadedSubtitle>{if(!isTauriDesktop())throw new Error('Subtitle downloads are currently managed from the desktop server app.');return invoke<DownloadedSubtitle>('subtitle_download',{mediaId,fileId,language});}
