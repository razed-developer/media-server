...
export async function setReaction(userId:string,targetType:SocialTargetType,targetKey:string,reaction?:string):Promise<ReactionEntry[]>{if(!isTauriDesktop())throw new Error('Reactions are currently available from the desktop server app.');const result=await invoke<ReactionEntry[]>('user_reaction_set',{userId,targetType,targetKey,reaction:reaction??null});window.dispatchEvent(new Event('onyx-household-feed-changed'));return result;}
